...
def fedot_method(data_w_nan, country, year, atc, tech):
	...
	pd.DataFrame(without_gap_forward).to_csv('data/'+str(year)+'/'+country+'/fedot/'+atc+'_'+tech+'_filled_forward.csv',
											 sep='\t', encoding='utf-8', index=False)
	pd.DataFrame(without_gap_bidirect).to_csv('data/'+str(year)+'/'+country+'/fedot/'+atc+'_'+tech+'_filled_bidirect.csv',
											  sep='\t', encoding='utf-8', index=False)
